...
В начале `<style>` внутри `src/index.html` есть блок **«ПАНЕЛЬ НАСТРОЙКИ»** —
все размеры, которые обычно хочется покрутить, собраны там в одном месте и
подписаны. Менять нужно только его, искать значения по всему файлу не надо.

| Переменная | За что отвечает |
|---|---|
| `--tl-media-width` | ширина картинки в «Карьерном пути»; меньше % — меньше картинка |
| `--tl-media-ratio` | пропорции картинки: `16/9`, `4/3`, `1/1` |
| `--tl-media-zoom` | во сколько раз растёт при наведении; `1` — не растёт |
| `--tl-media-zoom-speed` | скорость роста |
| `--fs-h1`, `--fs-h2` | имя на первом экране и заголовки разделов |
| `--fs-base`, `--fs-lede`, `--fs-nav` | базовый текст, вступление, меню |
| `--fs-tl-*` | шрифты раздела «Карьерный путь» |
| `--fs-about-*` | шрифты раздела «Про меня» |
| `--fs-contact-value` | телефон, e-mail, адрес |
| `--fade-out`, `--fade-in` | плавность перехода между языками |

Увеличение по наведению включается только там, где есть мышь
(`@media (hover:hover)`) — на телефоне картинка не «прыгает» и не вылезает за
край экрана.
